...
public abstract class RssManager {
    ...
    public static void clearAll(Context context) {
        ContentResolver cr = context.getContentResolver();
        cr.delete(Db.URI_RSS, null, null);
    }

    public static void clear(Context context, List<Rss> rssList) {
        ContentResolver cr = context.getContentResolver();
        Cursor cursor = cr.query(Db.URI_RSS, null, null, null, null);
        if (cursor != null && cursor.moveToFirst()) {
            do {
                Rss rss = new Rss(cursor);
                boolean has = false;
                for (Rss newRss : rssList) {
                    if (newRss.getRssId().equalsIgnoreCase(rss.getRssId())) {
                        has = true;
                        break;
                    }
                }
                if (!has) {
                    cr.delete(Db.URI_RSS, Db.Rss.RSS_ID + "=?", new String[] {rss.getRssId()});
                }
